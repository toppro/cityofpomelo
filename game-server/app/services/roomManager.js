 ...
 * Module dependencies
 */
var Room = require('../domain/entity/room');
...
var addSomeRobots2Room = function(roomObj, data) {
  if(!roomObj) {
    return;
  }

  var robotsNum = consts.ROOM.MAX_MEMBER_NUM - roomObj.getRealPlayerNum();
  // utils.myPrint('robotsNum = ', robotsNum);

  for(var i = 0; i < robotsNum; i++) {
    data.playerId = consts.ROOM.MAX_RANDOM_NUM + i;
    data.playerName = consts.ROOM.PREFIX_4_ROBOT + (i + 1);
    data.isRobot = true;
    roomObj.addPlayer(data);
  }
